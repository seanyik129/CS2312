...
public class Main{
    ...
    public static void main(String [] args) throws FileNotFoundException
    {
        ...
        if (students.length % numberOfTeams != 0) {
        ...
        teams = createTeams(students, numberOfTeams);//Call the private method (one of the 3 methods in Main_Lab08Q00.java) to create the teams for students: createTeams(students);
...
//        System.out.println(teams[4].toString());
        ...
        assignments = decideTasks(teams, in, numberOfTeams);//Call the private method (one of the 3 methods in Main_Lab08Q00.java) to input the tasks for each team: decideTasks(teams, in);

        String[] taskName = new String[numberOfTeams];
        for (int i = 0; i < numberOfTeams; i++) {
            taskName[i] = assignments[i].getTaskName();
        }

        Arrays.sort(taskName);
...
//        Assignment.printTaskTeam("Lab09", assignments);
    ...
    private static Team[] createTeams(Student[] students, int numberOfTeams)
    {
        Team[] result; //Define an array of teams to store the result: Team[] result;
        result = new Team[numberOfTeams]; //Create the array: new Team[5];

        //Create the 5 teams
        char c = 'A';
        int numberOfMember = students.length / numberOfTeams;
        int index = 0;
        for (int i = 0; i < numberOfTeams; i++) {
            result[i] = new Team("Team " + c, Arrays.copyOfRange(students, index, index + numberOfMember));
...
//        result[4] = new Team("Team E", Arrays.copyOfRange(students, 24,30));
    ...
    private static Assignment[] decideTasks(Team[] teams, Scanner in, int numberOfTeams)
    {
        Assignment[] assignments; //Define an array of assignments to store the result: Assignment[] assignments;
        assignments = new Assignment[numberOfTeams]; //Create the array: new Assignment[5];

        System.out.println();
        System.out.print("Enter " + numberOfTeams + " task names (eg. \"Lab05 Lab06 Lab07 ..\"): ");
        String[] task = new String[numberOfTeams];
        for (int i = 0 ; i < numberOfTeams; i++) {
            task[i] = in.next();
        }
